...
import com.example.juliantanausuvillar.activities_permisos_list.app.preferences
...
class SharedPreferencesActivity : AppCompatActivity() {
    ...
    private fun setValuesSharedPreferences(){
        if (editTextName.text.toString().isNotEmpty() && editTextAge.text.toString().isNotEmpty()){
            ...
            Toast.makeText(this, "Please fill the name and the afe before saving", Toast.LENGTH_SHORT).show()
        }
    }

    private fun cleanEditTexts(){
        // dos formar para hacer lo mismo -> limpiar
